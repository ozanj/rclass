...
\medskip
```{r results="hide"}
rename(df_event, zip = determined_zip, titlei = titlei_status_pub)
names(df_event)
```
Variable names do not change permanently unless we combine rename with assignment
\medskip
```{r results="hide"}
rename_event <- rename(df_event, zip = determined_zip, titlei = titlei_status_pub)
names(rename_event)
rm(rename_event)
```
...
```{r}
select(df_event,instnm,event_date,eventtype,event_state,med_inc)
```
### Select variables using \hlgc{select()} function
...
```{r}
event_small <- select(df_event,instnm,event_date,eventtype,event_state,med_inc)
event_small
```
...
df_event$eventtype[6:10]
```
- can also print multiple variables using \hlgc{combine()} function
```{r}
c(df_event$event_state[1:5],df_event$eventtype[1:5])
```
...
```{r}
class(df_event$eventtype)
str(df_event$eventtype)
table(df_event$eventtype)
...
```{r}
count(filter(df_event, eventtype == "public hs", event_state =="CA"))
...
```{r eval=FALSE, echo=FALSE}
count(filter(df_event, univ_id == 215293, eventtype == "private hs" | eventtype == "public hs"))
```
3. Count the number of recruiting events by Pitt at public or private high schools located in the state of PA
```{r eval=FALSE, echo=FALSE}
count(filter(df_event, univ_id == 215293, eventtype == "private hs" | eventtype == "public hs", event_state == "PA"))
```
...
```{r eval=FALSE, echo=FALSE}
count(filter(df_event, univ_id == 215293, eventtype == "public hs", event_state != "PA", med_inc < 100000))
```
...
```{r eval=FALSE, echo=FALSE}
count(filter(df_event, univ_id == 215293, event_state != "PA", (eventtype == "public hs" & med_inc >= 100000) | eventtype == "private hs"))
```
...
count(filter(df_event,eventtype == "public hs", fr_lunch<50))
#visits to public HS, where free/reduced lunch missing
count(filter(df_event,eventtype == "public hs", is.na(fr_lunch)))
#visits to public HS, where free/reduced is less than 50 OR is missing
count(filter(df_event,eventtype == "public hs", fr_lunch<50 | is.na(fr_lunch)))
```
...
select(arrange(df_event, univ_id, desc(g12)),instnm,eventtype,event_date,g12)
```
...
select(arrange(df_event, univ_id, desc(event_date), school_id),instnm,event_date,eventtype,school_id)

#by university, date, descending school id
select(arrange(df_event, univ_id, desc(event_date), desc(school_id)),instnm,event_date,eventtype,school_id)
```
Can sort by \hlgc{is.na} to put missing values first
```{r}
select(arrange(df_event, univ_id, desc(event_date), desc(is.na(school_id))),instnm,event_date,eventtype,school_id)
```
