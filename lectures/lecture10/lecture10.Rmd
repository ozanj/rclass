...
# Modfifying object vs. creating new object
...
The new object will contain the mean for each variable, so the new object should be a numeric vector with number of elements (length) equal to the number of variables

Grolemund and Wickham recommend creating the new object __prior__ to creating the loop
```{r}
#this code creates a numeric object called output
...
could use "rescale example" from 21.3 of Wickham

### Count of events for each university

This would use the df_event dataframe

### Load recruiting data for practical loop tasks
...
First, let's accomplish task outside of a loop for one university
...
- next, loop should be able to work with the variables that start "inst_..." and "visits_by_..."
```{r, results="hide"}
names(df_pubhs)
for (i in seq_along(instnm)) {
  cat("\n","i=",i,sep="",fill=TRUE)
...
- for each university, keep obs for high schools in same state as the university
...
for (i in seq_along(instnm)) {
  ...
  #create object called inst_var; value is "inst_id" (e.g., "inst_166629")
  inst_var <- paste("inst_",instnm[[i]],sep="")
  inst_var <- glue::glue({inst_var})
  #print(inst_var)
  #print(eval(inst_var))
  
  #keep obs in same state as the university [THIS CODE DOESN'T WORK]
  #df_pubhs %>% filter(state_code==glue::as_glue(inst_var)) %>% count(state_code)
  df_pubhs %>% filter_(glue::glue("state_code=={inst_var}")) %>% count(state_code)
  #print(df_pubhs %>% filter_(glue::glue("state_code=={inst_var}")) %>% count(state_code))
...
#base r aproach
for (i in seq_along(instnm)) {
  param <- paste("inst_", instnm[[i]],sep="")
  print(param)
  tempdf<-subset(df_pubhs, df_pubhs$state_code==df_pubhs[[param]])
}
names(tempdf)
tempdf %>% count(state_code)
