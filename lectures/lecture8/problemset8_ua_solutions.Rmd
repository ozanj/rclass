...
[Here](https://forms.gle/tGPZ8AnnPsok1T6CA)  
...
1. Tidying untidy data: reshaping from long to wide
    - this will be the longest part of the problem set because it is very common that data we find "in the wild" needs to be reshaped long to wide before it is tidy
...
- What is the difference between the terms "unit of analysis" [our term; not necessarily used outside this class] and "observational level" [A Wickham term]?  
    ...
    - ANSWER: \textcolor{red}{\textbf{/0.5}}  

__Wickham defines "observational level" as what each observation should represent in a tidy dataset (i.e., it is a data concept), whereas Ozan defines "unit of analysis" as what each row in the data actually represents (i.e., refers to data structure).__ 

 

- What are the three rules of tidy data?  

  __1. Each variable must have its own column.__ 
  
  __2. Each observation must have its own row.__  
  
  __3. Each value must have its own cell.__
...
- Create three different data frame objects based on the data frame `age_f16_allvars_allobs`
    - A dataframe `all_obs` that has fewer variables than `age_f16_allvars_allobs` but the same number of observations 
        - this data frame has the most complex structure; we'll reshape this one last
    ...
    - A dataframe `levstudy1_obs` that has fewer variables than `age_f16_allvars_allobs` and keeps observations where "level of study" equals `1` (1. All Students total)
        - this data frame has the second simplist structure; we'll reshape this one second
- Questions related to reshaping `agegroup1_obs`
- Questions related to reshaping `levstudy1_obs`
- Questions related to reshaping  `all_obs`
...
agegroup1_obs_tidy <- agegroup1_obs %>% 
  pivot_wider(names_from = levstudy, 
               values_from = efage09)
...
#add value labels to level
attributes(agegroup1_obs$levstudy)
agegroup1_obs_v2 <- agegroup1_obs %>% 
...
agegroup1_obs_tidy_v2 <- agegroup1_obs_v2 %>% 
...
Print a few observations of `agegroup1_obs_tidy_v2`; Why is this data frame preferable over `agegroup1_obs_tidy`?  
    ...
    - YOUR ANSWER HERE: more intuitive variable names
```{r}
...
Tidy the data frame `levstudy1_obs` and create a new object `levstudy1_obs_tidy` (it is up to you whether you want to create character version of the variable `agegroup` prior to tidying. If you don't you'll have very long variable names after reshpaing) then print a few observations
...
levstudy1_obs_v2 <- levstudy1_obs %>% 
...
levstudy1_obs_tidy <- levstudy1_obs_v2 %>% 
...
- Becasue we have not yet covered regular expressions, rename the `agegroup` and `level` variables so they have consistent formatting. Run the below code, which creates character versions of these two variables and then uses the `unit()` function to combine these two variables into a single variable. This code will create a new object all_obs_temp.
...
```{r}
all_obs %>% 
    pivot_wider(names_from = c(levstudy, agegroup),
                values_from = efage09)
```
