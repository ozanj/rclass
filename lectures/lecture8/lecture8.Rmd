...
if(!file.exists('rclass_folder.png')){
  download.file(url = 'https://github.com/ozanj/rclass/raw/master/lectures/lecture8/rclass_folder.png', 
...
if(!file.exists('rclass_folder_contents.png')){
  download.file(url = 'https://github.com/ozanj/rclass/raw/master/lectures/lecture8/rclass_folder_contents.png', 
...
__No class next Friday (11/23)__  

- Problem set #8 is due on Friday (11/23)  
- Problem set #9 will be assigned the following week, Friday (11/30) and will be due on Friday (12/7)  


__Questions on Problem set #7?__ 
...
- Download and unzip data folder we are using for this lecture [here](https://github.com/ozanj/rclass/raw/master/lectures/lecture8/lecture8_data.zip) 
...
| **Key** | **Description** |
| ------ | -------- |
| ~    | tilde is a shortcut for mac user's home directory (mine is my name pm)   |
| ../    | moves up a level   |
| ../../    | moves up two level   |
...
Refer to the `readr` package [pdf](https://cran.r-project.org/web/packages/readr/readr.pdf) for more detailed information on each function.   
- Note: All of these functions follow similar syntax/rules in `readr`
...
### readr arguments   
`read_csv(file, col_names = TRUE, col_types = NULL, na = c("", "NA"), comment = "", skip = 0, n_max = Inf,)`
...
### read_csv column specification, part 1
...
### readr column specification, part 2  
...
read_csv("a,b,c
          1,2,3
         4,5,6", col_names = c("column 1", "column 2", "column 3"), skip = 1
         )
```
...
__Usage (i.e., syntax)__: `as_factor(x, ..., only_labelled = TRUE)`
...
- `levels`: How to create the levels of the generated factor:  
    - "default": uses labels where available, otherwise the values. Labels are
sorted by value.  
...
__Example__: Change labelled columns to factor
```{r, results="hide"}
as_factor(df, only_labelled = TRUE) 
```
...
download.file(url = "http://www.equality-of-opportunity.org/data/race/table_1.csv",
              destfile = "../../../../table_1.csv", mode = "wb") 
```
...
names(ic2017_ay) <- tolower(names(ic2017_ay)) #lowercase column names
