...
Using `wwwlist`, create data frame with three variables
```{r, results = "hide"}
...
XIN CREATE SHORT STUDENT EXERCISES AND SOLUTIONS WHERE STUDENTS PRACTICE USING ATTRIBUTE() AND ATTR() ON DATA FRAME THAT IS A SUBSET (A FEW VARIABLES) OF THE DATA FRAME WWLIST

EXERCISES SHOULD TAKE 5-10 MINUTES TO COMPLETE
...
```{r}
wwlist$ethn_code_fac[1:3] # print first few obs of ethn_code_fac
```
...
XIN - BELOW FREQUENCY COUNT LOOKING ODD IN PDF. CAN YOU FIX?
- Default setting is to show variable __values__ not __value labels__
```{r}
df %>% count(edu)
```
...
XIN ADD ONE OR TWO SLIDES WHERE STUDENTS APPLY SET_VARIABLE_LABEL AND SET_VALUE_LABEL TO VARIABLES FROM DATA FRAME WWLIST 
...
XIN ADD ONE OR TWO EXERCISE QUESTIONS WHERE STUDENTS APPLY SET_VARIABLE_LABEL AND SET_VALUE_LABEL TO VARIABLES FROM DATA FRAME WWLIST 
...
XIN - CAN YOU FIGURE OUT HOW TO CONVERT ALL FACTOR VARIABLES TO LABELLED VARIABLES, AND MAKE IT SO THE FACTOR LEVELS FROM FACTOR VARIABLE BECOME THE VALUE LABELS IN LABELLED VARIABLE?
```{r, eval=FALSE}
glimpse(hsls)
glimpse(hsls_f)

hsls_f_l <- to_labelled.factor(x = hsls_f)
```
