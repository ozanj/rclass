...
### Using `html_document` rather than `beamer_presentation` output for the next two lectures (lecture 6 and 7)
...
- To see formatted version of lecture, open `lecture6.html` or knit and follow along in that version
...
- A link to mid-term course survey will be posted at beginning of lecture 6 problem set. It is short. please complete it!
- focus is on how class is working for you; what is going well; how can class be improved

### Reading to do before next class

- Work through slides from lecture 6 that we don't get to in class
    - Assuming we get through the section on __tidying data__, the rest of the lecture focuses on missing values created by tidying and it is fine for you to read this __after__ you finish problem set
...
1. Use `group_by` to perform calculations separately within groups and then use `summarise` to create an object with one observation per group. Examples:
    - Creating objects containing summary statistics that are basis for tables and graphs (focus of lecture 4)
    - Creating student-transcript level GPA variable from student-transcript-course level data (focus of lecture 5 problem set)
...
Before we define "tidy data", we will spend significant time defining and discussing the some core terms/concepts about datasets. 
...
Identifying which combination of variables uniquely identifies rows in a data frame helps you understand the "structure" of your dataset
...
3. Transform untidy to tidy
    - __names__ of the set of columns that represent values, not variables in your untidy data 
        - `tot_2000,tot_2010,tot_2011,tot_2012`
    - __key variable__ : variable name you will assign to columns you are gathering from the untidy data
        - This var doesn't yet exist in untidy data, but will be a variable name in the tidy data
        - In this example, __key__ var is year 
    - __value variable__: The name of the variable that will contain values in the tidy dataset you create and whose values are spread across multiple columns in the untidy dataset
        - This var doesn't yet exist in untidy data, but will be a variable name in the tidy data
        - in this example, __value__ var is total_students
...
### Gathering with multiple value variables

Working with data from same data frame `table204_10`, but this time we have variables for total enrollment __and__ variables for total number of students on free/reduced lunch
```{r}
totfrl <- table204_10 %>% 
...
__Task__: Tidy the dataset `totfrl` by converting columns to rows. Basically, we want to create "long" dataset with a `year` variable, a `total_students` variable, and a `frlunch_students` variable


- __names__ of the set of columns that represent values, not variables in your untidy data 
    - `tot_2000,tot_2010,tot_2011,tot_2012,frl_2000,frl_2010,frl_2011,frl_2012`
- __key variable__ : variable name you will assign to columns you are gathering from the untidy data
    - This var doesn't yet exist in untidy data, but will be a variable name in the tidy data
    - In this example, __key__ var is year 
- __value variable__: The name of the variable that will contain values in the tidy dataset you create and whose values are spread across multiple columns in the untidy dataset
    - in this example, there would be two __value__ variables is `total_students` and `frlunch_students`

Unfortunately, `gather()` function not set up to work with multiple __value__ variables, so we cannot do this task with our existing tools. 

- but you could find several approaches through Googling

```{r, eval=FALSE}

#this code doesn't do what we want it to
totfrl %>% 
  gather(tot_2000,tot_2010,tot_2011,tot_2012,frl_2000,frl_2010,frl_2011,frl_2012, 
         key = year, value = total_students)
```
...
ipeds_hc_multi_val %>% head(n=10)
```
Unfortunately, `spread()` function not set up to deal with multipel __value__ variables

- completing this task would require alternative approaches/additional programming concepts
- You can google this, but not required for homework
