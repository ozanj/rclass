...
- `summarise()` [with or without `group_by()`] can be helpful for creating summary statistics that are the inputs for tables or graphs you create
...
__Description__: `summarise()` calculates across rows; then collapses into single row

- `summarise()` create scalar vars summarizing variables of existing data frame
...
```{r}
df_event %>%
  summarise(avg_inc = mean(med_inc, na.rm = TRUE), # all events
...
### Using `summarise()` to create descriptive statistics table
...
df_event %>% group_by(instnm, event_type) %>% summarise(num_events=n()) %>% str()
```
...
XIN - WHY ISN'T .DROP = FALSE [BELOW] KEEPING ROWS FOR EMPTY GROUPS?
```{r, results="hide"}
...
df_event %>% group_by(instnm, event_type, event_inst, .drop = FALSE) %>% 
  summarise(num_events=n()) 
```
...
event_berk %>% group_by(event_type, event_inst) %>% 
  summarise(num_events=n()) # same same
...
```{r}
event_berk %>% group_by(event_type, event_inst) %>% 
  summarise(max_inc = max(med_inc)) # oops, we forgot to remove NAs!
...
### Summarize multiple columns with summarize_all(), summarize_at(), summarize_if()

```{r, eval=FALSE, include=FALSE}
?summarise_all
```
__Description__. The scoped variants of `summarise()` make it easy to apply the same transformation to multiple variables. There are three variants:
...
```{r}
event_berk %>% select(event_type, event_inst,pct_white_zip,med_inc) %>%
  group_by(event_type,event_inst) %>%
  summarise_all(.funs = list(mean,min,max)) # forgot , na.rm = TRUE
# add na.rm = TRUE
event_berk %>% select(event_type, event_inst,pct_white_zip,med_inc) %>%
  group_by(event_type,event_inst) %>%
  summarise_all(.funs = list(mean,min,max), na.rm = TRUE)
#explicitly name variables
event_berk %>% select(event_type, event_inst,pct_white_zip,med_inc) %>%
  group_by(event_type,event_inst) %>%
  summarise_all(.funs = list(avg=mean,low=min,high=max), na.rm = TRUE)
