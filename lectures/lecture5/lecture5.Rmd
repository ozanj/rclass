...
- but in thise cours we will work with classes that have been created by others
...
wwlist <- wwlist %>% mutate(ethn_code_fac = factor(ethn_code)) # create factor var; tidyverse approach
...
### Creating factor variables from characterer variables or from integer variables
...
class(hsls$s3classes)
hsls %>% select(s3classes,s3clglvl) %>% var_label #show variable label
hsls %>% select(s3classes) %>% count(s3classes) # freq table, values
hsls %>% select(s3classes) %>% count(s3classes) %>% as_factor() # freq table, value labels
```

filter specific values
```{r, results="hide"}
hsls %>% select(s3classes) %>% filter(s3classes==-8) %>% count() # -8 = unit non-response
hsls %>% select(s3classes) %>% filter(s3classes==1) %>% count() # 1 = yes
```

### Student exercise

PATRICIA CREATE STUDENT EXERCISE; SHOULD INVOLVE FREQUENCY TABLES AND FILTERING; EXPERIMENT WITH FILTERS THAT FITLER MULTIPLE VALUES OF VARIABLE RATHER THAN JUST ONE
...
```{r}
#hsls %>% var_label()
hsls_small <- hsls %>% select(stu_id,x3univ1,x3sqstat,x4univ1,x4sqstat,s3classes,       s3work,s3focus,s3clgft,s3workft,s3clgid,s3clgcntrl,s3clglvl,s3clgsel,s3clgstate,s3proglevel,
