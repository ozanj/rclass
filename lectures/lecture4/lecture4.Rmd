...
Use one code chunk for this exercise. You could tackle this a step at a time until you could run the entire code chunk answering all parts of this question. Create your own variable names.  
...
1. Using the `event_berk` object, group by `instnm`, `event_inst`, & `event_type`.  
    1. Create vars for number non_missing for these racial/ethnic group (`pct_white_zip`, `pct_black_zip`, `pct_asian_zip`, `pct_hispanic_zip`, `pct_amerindian_zip`, `pct_nativehawaii_zip`)  
...
Task: Investigate visit patterns for each event type.

Using the example from the previous slide as a guide, what is the value of event data for:  
- the first, second, third, fourth, and fifth event for each event type?  
