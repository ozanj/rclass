...
### Compare tidyverse to subset() from base R, extracting columns (variables)

XIN - CREATE EXERCISES [AND SOLUTIONS] FOR STUDENTS BASED ON subset(). FOR EACH QUESTION, ASK STUDENTS TO PERFORM THE SAME TASK USING subset() and filter(). USE BELOW TWO SLIDES TO GET YOU STARTED [BUT GOOD TO THINK OF A COUPLE ADDITIONAL QUESTIONS]. YOU CAN REMOVE STUFF ABOUT SUB-SETTING OPERATORS BECAUSE WE HAVEN'T GOTTEN TO THOSE YET IN THE LECTURE.
...
### Compare tidyverse to subset() from base R, extracting observations
...
XIN - CAN YOU MAKE A SHORT SET OF EXERCISES [AND SOLUTIONS] BASED ON SUBSETTING DATA FRAMES USING []
...
#num obs event_type=="public hs" & med_inc is not NA & med_inc >= $50,000
nrow(subset(df_event, event_type == "public hs" & is.na(med_inc)==0 & med_inc>=50000))
...
### Subsetting Data Frames with [] and $/STUDENT EXERCISES

XIN - CREATE STUDENT EXERCISES. CAN USE THIS SLIDE FOR ONE OF EXERCISES AND MAKE ONE OR TWO  MORE. FOR EACH QUESTION, ASK STUDENTS TO ACCOMPLISH USING (1) [] COMBINED WITH $; (2) FILTER(); AND (3) SUBSET()
