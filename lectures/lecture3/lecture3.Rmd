---
title: "Lecture 3: Pipes and creating variables using `mutate()`"
...
## Finish lecture 2, filter and arrange (i.e., sort)
...
- Off-campus recruiting project; one obs per university, recruiting event

Object `df_event`

- Off-campus recruiting project; one obs per high school (visited and non-visited)

\medskip Work through lecture 2 slides on `filter()` and `arrange()`
...
__Task__: Create frequency table of `school_type` for non first-gen prospects from WA
...
PATRICIA CAN YOU CREATE A COUPLE STUDENT EXERCISES FOR PIPES
...
school_v2_temp$pct_fr_lunch <- school_v2_temp$num_fr_lunch/school_v2_temp$total_students
...
- Create 0/1 indicator of school has median income greater than $100,000
...
school_v2 %>% filter(is.na(med_inc)) %>% count(med_inc) # shows number of obs w/ missing med_inc
```
Create variable
```{r}
...
- Create 0/1 indicator variable `in_state_berkeley` that equals `1` if the high school is in the same state as UC Berkeley (i.e., `state_code=="CA"`)
...
- create 0/1 indicator `berkeley_and_irvine` of whether a school got at least one visit from UC Berkeley __AND__ from UC Irvine
```{r, results="hide"}
school_v2 %>% 
...
- create 0/1 indicator `berkeley_or_irvine` of whether a school got at least one visit from UC Berkeley __OR__ from UC Irvine
```{r, results="hide"}
school_v2 %>% 
...
- `...`	Specifications for recode, of the form `current_value = new recoded value` 
...
- in wwlist object, recode the variable psat_range or recode the variable ethn_code
```{r}
str(wwlist$psat_range)
wwlist %>% count(psat_range)
str(wwlist$ethn_code)
wwlist %>% count(ethn_code)
```
- in object school_v2, recode the variable event_type
...
## Alternative approach to creating 0/1 indicators

### Creating indicators for `school_v2` data frame

PATRICIA - I WROTE THESE BELOW SLIDES IN SUMMER BEFORE I KNEW ABOUT `ifelse()`; YOU MAKE A RECOMMENDATION TO ME ABOUT WHETHER THESE SLIDES SHOULD BE CUT
...
str(school_v2_temp$lt50pctwhite)
```
Create 0/1 integer indicator rather than logical indicator 
```{r}
df_schoolv2_temp <- school_v2 %>% mutate(lt50pctwhite = as.integer(pct_white<50))
...
school_v2_temp %>% filter(visits_by_berkeley>0) %>% select(visits_by_berkeley,visits_by_berkeley)
school_v2_temp %>% filter(visits_by_berkeley==0) %>% select(visits_by_berkeley,yesvis_berkeley)
